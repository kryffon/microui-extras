...
main :: proc() {
	...
	ctx.text_width = mux.get_text_width
	ctx.text_height = mux.get_text_height

	FontConfig :: struct {
		path, name: string,
		size:       i32,
	}
	font_configs := [?]FontConfig {
		{"", "default", 18},
		{"./fonts/FiraCode-Regular.ttf", "FiraCode", 14},
		{"./fonts/Cousine-Regular.ttf", "Cousine", 14},
		{"./fonts/DroidSans.ttf", "DroidSans", 14},
		{"./fonts/Karla-Regular.ttf", "Karla", 14},
		{"./fonts/ProggyClean.ttf", "ProggyClean", 12},
		{"./fonts/ProggyTiny.ttf", "ProggyTiny", 10},
	}

	for cfg, i in font_configs {
		font := mux.load_default_font() if i == 0 else mux.load_font(cfg.path, cfg.size)
		append_nothing(&state.fonts)
		state.fonts[len(state.fonts) - 1] = {
			atlas_texture = load_font_texture(&font),
			font          = font,
			name          = cfg.name,
		}
		...
		when ODIN_DEBUG {
			stbi.write_png(
				rl.TextFormat("%s_font_altas.png", cfg.name),
				font.width,
				font.height,
				1,
				raw_data(font.alpha),
	...
	defer {
		for &f in state.fonts {
			mux.destroy_font(&f.font)
			rl.UnloadRenderTexture(f.atlas_texture)
		}
		delete(state.fonts)
	}

	state.mu_ctx.style.font = mu.Font(rawptr(&state.fonts[state.current_font_id].font))
...
load_font_texture :: proc(font: ^mux.Font) -> rl.RenderTexture2D {
	atlas_texture := rl.LoadRenderTexture(font.width, font.height)

	image := rl.GenImageColor(font.width, font.height, rl.Color{0, 0, 0, 0})
	defer rl.UnloadImage(image)

	for alpha, i in font.alpha {
		x := i32(i) % font.width
		y := i32(i) / font.height
...
render :: proc(ctx: ^mu.Context) {
	render_texture :: proc "contextless" (
		...
		rl.DrawTextureRec(
			texture = state.fonts[state.current_font_id].atlas_texture.texture,
			source = {f32(src.x), f32(src.y), f32(src.w), f32(src.h)},
			position = {f32(dst.x), f32(dst.y)},
			tint = color,
		)
	}
	...
	font := &state.fonts[state.current_font_id].font
	...
	for variant in mu.next_command_iterator(ctx, &command_backing) {
		...
		case ^mu.Command_Text:
			cursor := mu.Rect{cmd.pos.x, cmd.pos.y + font.yoffset, 0, 0}
			for ch in cmd.str do if ch & 0xc0 != 0x80 {
				src, dst := mux.get_text_coords(font, ch, &cursor)
		...
		case ^mu.Command_Icon:
			src := mux.get_src_rect(font.atlas[cmd.id])
			x := cmd.rect.x + (cmd.rect.w - src.w) / 2
			y := cmd.rect.y + (cmd.rect.h - src.h) / 2
			render_texture(state.screen_texture, mu.Rect{x, y, 0, 0}, src, to_rl_color(cmd.color))
...
all_windows :: proc(ctx: ^mu.Context) {
	...
	if mu.window(ctx, "Fonts", {660, 40, 200, 250}) {
		for &f, i in state.fonts {
			mu.layout_row(ctx, {-1})
			bopts: mu.Options
			if state.current_font_id == i do bopts = {.HOLD_FOCUS}
			if .SUBMIT in mu.button(ctx, f.name, .NONE, bopts) {
				state.current_font_id = i
				state.mu_ctx.style.font = mu.Font(rawptr(&f.font))
				write_log(fmt.tprintf("font switched to %q", f.name))
			}
		}
	}
}
